...
from .ReversibleInstanceNormalization import RevIN, SeriesDecomposition
...
class Forecaster(pl.LightningModule, ABC):
    def __init__(
        ...
        # Simple linear baseline: last-value persistence over horizon
        if linear_window:
            def _linear_baseline(y_context, pred_len: int, d_yt: int):
                # y_context: (B, L, d_yt); return repeat of last context step
                last = y_context[:, -1:, :]
                return last.repeat(1, pred_len, 1)
            self.linear_model = _linear_baseline
        else:
    ...
    def _compute_stats(
        ...
        # Minimal metric implementations to avoid external deps
        def _mae(a, b):
            return _np.mean(_np.abs(a - b))
        def _mse(a, b):
            return _np.mean((a - b) ** 2)
        def _mape(a, b):
            denom = _np.clip(_np.abs(a), 1e-8, None)
            return _np.mean(_np.abs((b - a) / denom)) * 100.0
        def _smape(a, b):
            denom = (_np.abs(a) + _np.abs(b)) / 2.0
            denom = _np.clip(denom, 1e-8, None)
            return _np.mean(_np.abs(b - a) / denom) * 100.0

        stats = {
            "mape": _mape(scaled_true, scaled_pred) / adj,
            "mae": _mae(scaled_true, scaled_pred) / adj,
            "mse": _mse(scaled_true, scaled_pred) / adj,
            "smape": _smape(scaled_true, scaled_pred) / adj,
            "norm_mae": _mae(true, pred) / adj,
            "norm_mse": _mse(true, pred) / adj,
        }
        return stats
