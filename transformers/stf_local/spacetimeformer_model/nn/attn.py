...
class TriangularCausalMask(torch.Tensor):
    def __new__(cls, B, L, device=None):
        mask = torch.ones((B, 1, L, L), dtype=torch.bool, device=device)
        mask = torch.triu(mask, diagonal=1)
        return torch.as_tensor(mask, dtype=torch.bool, device=device)


class ProbMask:
    def __init__(self, B, H, L_Q, index, scores, device=None):
        # Build a boolean mask of shape (B, H, L_Q, L_K) with False where allowed
        self.mask = torch.zeros((B, H, L_Q, scores.shape[-1]), dtype=torch.bool, device=device)
        # No additional masking beyond causal logic handled elsewhere
        # Keep API-compatible attr
